...
class TestSessionListCreateView(TestCase):
    ...
    def test_create_invalid_repo_url_returns_400(self):
        response = self.client.post(
            reverse("session-list-create"),
            {"repo_url": "not-a-url-or-path", "question": "What does this do?"},
            format="json",
        )
        self.assertEqual(response.status_code, 400)
        self.assertIn("repo_url", response.data)
    ...
    @patch("research_sessions.services.run_research_task")
    def test_create_with_local_path_returns_201(self, mock_task):
        mock_task.delay.return_value = None
        with tempfile.TemporaryDirectory() as tmp:
            response = self.client.post(
                reverse("session-list-create"),
                {"repo_url": tmp, "question": "What is in this repo?"},
                format="json",
            )
        self.assertEqual(response.status_code, 201)
        self.assertEqual(response.data["status"], "pending")
